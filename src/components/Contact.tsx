        ...
        <div className="grid lg:grid-cols-2 gap-14 xl:gap-24 items-start">
          ...
          <div className="flex flex-col gap-3 mt-4 lg:mt-12">
            {[
              ...
              <div key={i} className="flex items-center gap-5 p-5 border" style={{ backgroundColor: '#fff', borderColor: '#e8e4dc' }}>
                <div className="w-10 h-10 flex items-center justify-center shrink-0 rounded-full" style={{ backgroundColor: '#f0f5f1', color: '#214A32' }}>
                  {item.icon}
                </div>
                <div>
                  <div className="text-xs font-semibold tracking-widest uppercase mb-1" style={{ color: '#999' }}>{item.label}</div>
                  {item.href ? (
                    <a href={item.href} target={item.href.startsWith('http') ? '_blank' : undefined}
                      rel="noopener noreferrer"
                      className="text-sm font-medium hover:opacity-70 transition-opacity" style={{ color: '#1D1D1B' }}>
                      {item.value}
                    </a>
                  ) : (
                    <div className="text-sm font-medium" style={{ color: '#1D1D1B' }}>{item.value}</div>
                  )}
                </div>
              </div>
            ))}
